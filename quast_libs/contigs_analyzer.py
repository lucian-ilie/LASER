...
#        aligns.setdefault(mapping[8], []).append(mapping)
        line = line.split()
        assert line[2] == line[5] == line[8] == line[10] == '|', line
        aligns.setdefault(line[12], []).append((int(line[0]),int(line[1]),int(line[3]),int(line[4]),int(line[6]), int(line[7]), float(line[9]), line[11], line[12]))
    ...
    if qconfig.show_snps:
        ...
        for line in open_gzipsafe(show_snps_fpath):
            ...
            ref_nucl = line[1]
            ctg_nucl = line[2]
            type = 'I' if ref_nucl == '.' else ('D' if ctg_nucl == '.' else 'S')
...
#                snps.setdefault(ref, {}).setdefault(ctg, {})[pos] = [SNP(ref_pos=pos, ctg_pos=loc, ref_nucl=line[1], ctg_nucl=line[2])]
            snps.setdefault(ref, {}).setdefault(ctg, {}).setdefault(pos,[]).append((ref, ctg, pos, loc, ref_nucl, ctg_nucl, type))
            prev_line = line
    ...
    for chr_name, aligns in ref_aligns.iteritems():
        ...
        contigs = set([align[8] for align in aligns])
        ...
        if qconfig.is_combined_ref:
            ...
            for align in aligns:
                align_by_contigs[align[8]] += align[5]
