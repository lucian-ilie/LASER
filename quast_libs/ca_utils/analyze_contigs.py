...
from quast_libs.qutils import  icarus_report_str, _start, _end
    ...
    for contig, seq in fastaparser.read_fasta(contigs_fpath):
        ...
        if contig in aligns:
            for align in aligns[contig]:
                ...
                if 'N' in sub_seq:
                    ns_pos = [pos for pos in xrange(_start(align), _end(align)) if seq[pos] == 'N']
#                    ns_pos = [pos for pos in xrange(align.start(), align.end()) if seq[pos] == 'N']
            ...
            sorted_aligns = sorted(aligns[contig], key=lambda x: (score_single_align(x), x[5]), reverse=True)
...
#                    print >> ca_output.icarus_out_f, top_aligns[0].icarus_report_str()
                    print >> ca_output.icarus_out_f, icarus_report_str(top_aligns[0])
                    ref_aligns.setdefault(top_aligns[0][7], []).append(top_aligns[0])
...
#                            print >> ca_output.icarus_out_f, top_aligns[0].icarus_report_str(ambiguity=True)
                            print >> ca_output.icarus_out_f, icarus_report_str(top_aligns[0], ambiguity=True)
                            ref_aligns.setdefault(top_aligns[0][7], []).append(top_aligns[0])
                            if first_alignment:
                    ...
                    if qconfig.ambiguity_usage == "none":
                        ...
                        if len(the_best_set.indexes) < len(used_indexes):
                            ...
                            for idx in used_indexes:
                                ...
                                if idx not in the_best_set.indexes:
                                    print >> ca_output.icarus_out_f, icarus_report_str(align, is_best=False)
...
#                    print >> ca_output.icarus_out_f, the_only_align.icarus_report_str()
                    print >> ca_output.icarus_out_f, icarus_report_str(the_only_align)
                    if begin - 1:
                    ...
                    if (unaligned_bases >= qconfig.significant_part_size) and (ctg_len - unaligned_bases >= qconfig.significant_part_size):
                        ...
                        if qconfig.meta:
                            contigs_with_istranslocations += check_for_potential_translocation(seq, ctg_len, real_aligns,
                                                                                               ca_output.stdout_f)
                    ref_aligns.setdefault(the_only_align[7], []).append(the_only_align)
                else:
                    #Sort real alignments by position on the contig
                    sorted_aligns = sorted(real_aligns, key=lambda x: (_end(x), _start(x)))
...
#                            print >> ca_output.icarus_out_f, align.icarus_report_str()
                            print >> ca_output.icarus_out_f, icarus_report_str(align)
