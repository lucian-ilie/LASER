...
# Added function - nilesh
def icarus_report_str(align, ambiguity='', is_best='True'):
        return '\t'.join(str(x) for x in [align[0], align[1], align[2], align[3], align[7], align[8], align[6], ambiguity, is_best])

def _start(align):
        """Return start on contig (always <= end)"""
        return min(align[2], align[3])

def _end(align):
        """Return end on contig (always >= start)"""
        return max(align[2], align[3])

def __str__(align):
        return ' '.join(str(x) for x in [align[0], align[1], '|', align[2], align[3], '|', align[4], align[5], '|', align[6], '|', align[7], align[8]])

def short_str(align):
        return ' '.join(str(x) for x in [align[0], align[1], '|', align[2], align[3], '|', align[4], align[5]])
